const ONEPROJECT = (function() {
  ...
  const eventAdder = node => (eType, cb) =>
    eType.forEach(el => node.addEventListener(el, cb));
  const nodeCounter = (node, pr) =>
    pr
      ? pr.querySelectorAll(node).length
      : document.querySelectorAll(node).length;

  function addNewInput() {
    let todoLi = nodeAdder("li", wip);
    setAttrs(nodeAdder("input", todoLi), {
      type: "checkbox",
      disabled: "true"
    });
    setAttrs(nodeAdder("input", todoLi), {
    ...
    setAttrs(nodeAdder("a", todoLi), {
      href: "#",
      class: "btn-del btn-del-wip"
    });
  ...
  function liMover(target) {
    ...
    if (target.className === "done") {
      ...
      if (document.querySelectorAll("ul").length === 1) {
        nodeAdder(
          "button",
          setAttrs(nodeAdder("ul", projectArea), { class: "done" })
        );
        setAttrs(nodeAdder("div", document.querySelector("ul[class=done]")), {
          class: "done",
          style: "display: none;"
        });
      }
      var completedLi = nodeAdder(
        "li",
        document.querySelector("ul[class=done] div")
      );
      setAttrs(nodeAdder("input", completedLi), {
        type: "checkbox",
        checked: "",
        class: "done"
      });
      nodeAdder("span", completedLi).innerHTML = todoValue;
      setAttrs(nodeAdder("a", completedLi), {
        href: "#",
        class: "btn-del btn-del-done"
      });
    }
    target.parentNode.remove();
    let liNumber = nodeCounter("ul[class=done] li")
      ? `(${nodeCounter("ul[class=done] li")})`
      : "";
    let doneDiv = document.querySelector("div[class=done]");
    let showHideMessage =
      doneDiv.style.display === "none" ? "보이기" : "숨기기";
    if (document.querySelector("ul[class=done]"))
      document.querySelector(
        "ul[class=done] button"
      ).innerText = `완료된 항목 ${liNumber} ${showHideMessage}`;
  }
  ...
  function liRemover(target) {
    switch (target.classList[1]) {
      case "btn-del-wip":
        ...
        break;
      case "btn-del-done":
        if (nodeCounter("ul[class=done] li") === 1)
          document.querySelector("ul[class=done]").remove();
        target.parentNode.remove();
        let liNumber = nodeCounter("ul[class=done] li")
          ? `(${nodeCounter("ul[class=done] li")})`
    ...
    switch (e.type) {
      case "keydown":
        ...
        if (activeEl.type === "text" && activeEl.value) {
          if (
            activeEl.parentNode.nextSibling === null ||
  ...
  return eventAdder(document.querySelector("#background"))(
    ["keydown", "mousedown"],
    eventHandler
  );
})();
